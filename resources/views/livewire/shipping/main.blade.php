<div>
    ...
    <div class="flex flex-row">
        <div class=" flex-auto w-64 ">
            <x-shipping-tester.st-status-menu />
            <x-shipping-tester.st-filter />
            <!-- Tab สำหรับเลือกและเปลี่ยนหน้าได้ (ส่งค่า+if else)-->
            <div class="mb-4 border-b border-gray-200 dark:border-gray-700">
                <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" id="default-tab" data-tabs-toggle="#default-tab-content" role="tablist">
                    <li class="me-2" role="presentation">
                        <button class="inline-block p-4 border-b-2 rounded-t-lg hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300 aria-selected:text-main-color aria-selected:border-main-color aria-selected:font-bold" id="all-tab" data-tabs-target="#all" type="button" value="" role="tab" aria-controls="all" aria-selected="false">ทั้งหมด (<span> {{ 40 }}</span>)</button>
                    </li>
                    <li class="me-2" role="presentation">
                        <button class="inline-block p-4 border-b-2 rounded-t-lg hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300 aria-selected:text-main-color aria-selected:border-main-color aria-selected:font-bold" id="inno-tab" data-tabs-target="#inno" type="button" role="tab" aria-controls="inno" aria-selected="false">Organic Inno (<span> {{ 20 }}</span>)</button>
                    </li>
                    <li class="me-2" role="presentation">
                        <button class="inline-block p-4 border-b-2 rounded-t-lg hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300 aria-selected:text-main-color aria-selected:border-main-color aria-selected:font-bold" id="cosme-tab" data-tabs-target="#cosme" type="button" role="tab" aria-controls="cosme" aria-selected="false">Organic Cosme (<span> {{ 20 }}</span>)</button>
                    </li>
                </ul>
            </div>
            <div id="default-tab-content">
                <div class="hidden" id="all" role="tabpanel" aria-labelledby="all-tab">
                    <x-shipping-tester.st-list :stList="$st_list" :stDetail="$stDetail" :comType="0" />
                </div>
                <div class="hidden" id="inno" role="tabpanel" aria-labelledby="inno-tab">
                    <x-shipping-tester.st-list :stList="$st_list" :stDetail="$stDetail" :comType="1" />
                </div>
                <div class="hidden" id="cosme" role="tabpanel" aria-labelledby="cosme-tab">
                    <x-shipping-tester.st-list :stList="$st_list" :stDetail="$stDetail" :comType="2" />
                </div>
            </div>
        </div>

        <div class="rm-detail flex-auto w-32  hidden {{ !empty($stDetail) ? 'lg:flex' : '' }} p-3">
            <ul class="w-full p-2 text-sm font-medium text-gray-900 bg-white border border-gray-300 rounded-2xl dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                {{-- title --}}
                <li class="w-full px-4 py-2 border-b border-gray-200 rounded-t-lg dark:border-gray-600 text-main-color font-black text-sm">
                    <div class="flex flex-row justify-start content-center">
                        <svg class="fill-main-color w-20 h-20" viewBox="0 0 30 20" xmlns="http://www.w3.org/2000/svg">
